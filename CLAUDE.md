...
## A `createBelief` caller with more than one gift must actually train relevance, not just build the layers

**`generation/belief.js::createBelief` throws if `rho` is missing whenever more than one received layer is present — but a caller that supplies `rho` and then never calls `belief.witnessForm(context, form)` as it reads forward has satisfied the constructor without exercising the mechanism the constructor exists for. `witnessForm` is the only place `logW` (each gift's earned log-weight) ever changes; skip it and every gift's weight sits at its untouched initial value for the whole run, `shares()` softmaxes over N identical numbers, and the mixture is silently uniform — 1/N per gift, indistinguishable from "no gift is more relevant than any other" — regardless of any real relevance. `scripts/relevance.mjs` gets this right: its read loop calls `belief.witnessForm(ctx, tokens[i])` on the PRECEDING context, then `belief.readLayer.observe(seen, seen.length - 1)`, every form, in that order.**

**The incident this rule is named for** (`scripts/imagine.mjs`, 2026-08-19): the script's own cut-loop only ever called `.observe()`. Two separate bugs stacked on top of each other and hid one another. First, `makeReader`'s `createBelief({ layers: ... })` call never passed `rho` at all — a script-breaking `TypeError` on ANY run with more than one gift, including the script's own 3-gift default, so the RESULTS.md example this script's header advertises could not have been run against current `belief.js` without first hitting this. Fixed by declaring `RHO = 0.9995` (the same value `scripts/relevance.mjs` already uses for this exact parameter — not a fresh pick) and passing it through. Second, once that no longer crashed, the missing `witnessForm` call meant every gift's relevance was never actually earned. This was invisible at the default 3 gifts (a uniform 33/33/33 still "looks like" the mixture is doing something) and only became obvious when pushed to 38 gifts and measured directly via `relevanceReport()`: a flat ~1% share each, zero differentiation, at every one of 6 checkpoints through the book. Fixed by mirroring `relevance.mjs`'s causal witnessForm-then-observe pair inside the cut-loop.

**The general lesson, not specific to this script:** a belief with received layers has two independent pieces of state that both need training — the layers' own counts (`observe`/`train`) and the belief's cross-layer relevance weights (`witnessForm`). Building the first without the second produces a belief that generates without crashing, blends gifts without erroring, and reports plausible-looking percentages — every visible signal says it is working — while the actual relevance computation never ran. `relevanceReport().gated` is `true` whenever `received.length > 1`, but gated is not the same claim as trained; nothing currently distinguishes "gated and earned" from "gated and frozen at its initial log-weight of 0" short of checking whether the shares are suspiciously uniform. A caller wiring up a new multi-gift belief should call both `witnessForm` and `observe` per form from the start, and a reviewer of one should ask specifically whether `witnessForm` is called anywhere before trusting a `relevanceReport()`.

## Training many `belief.js` received layers at once trades nothing useful for real memory — batch the comparison instead

**Holding N fully-trained order-k layers in one `createBelief` simultaneously (a real gift plus its `shuffledGift` noise-floor twin, times N candidates) costs V8 heap proportional to the total COUNT of small tracking objects across all of them — a `{succ: Map, total: {v,t}}` entry per distinct n-gram context, per order, per layer — not proportional to the vocabulary's string bytes. Interning tokens to small integers before training does not fix an OOM caused by this, because it shrinks the bytes inside each key, not the number of keys or the number of nested Map/cell objects V8 has to track. If a caller only ever needs one layer's (or one pair's) OWN `relevanceReport().share` and nothing about the others, never all of them coexisting at once, the fix is structural: train the pair, read the number off it, let the pair go, build the next pair fresh.**

**The incident this rule is named for** (an ad hoc 38-real + 38-shuffled-floor relevance sweep against War and Peace, 2026-08-19, not a checked-in script): building all 76 received layers plus the read layer (order 4, 656k read forms, up to 90,000 forms per gift) into one `createBelief` OOM'd V8's default heap partway through training, and OOM'd again after interning every token to an integer first — same failure, same rough point in training, confirming the fix that had just worked for a different problem (shorter map keys) did nothing here. Rebuilding the same sweep to train ONE real layer + its shuffled twin + the belief scoring them, read `relevanceReport()`, and let all three go before the next pair, completed on the default heap with no flag: 182 seconds, 2.7GB peak RSS, same substantive finding as the all-at-once version (38 of 38 real gifts beat their own shuffled-vocabulary floor) — versus the all-at-once version needing a 12GB `--max-old-space-size` bump and still OOMing once even at that size before the read layer's own training loop was also de-duplicated out of the per-pair cost.

**Where this does and does not apply.** This is about PEAK memory during comparison, not about model quality — the batched version is not an approximation of the all-at-once one, it produces the identical `relevanceReport().share` per gift (each gift's share only ever depended on its own layer, the read layer, and `rho`, never on which OTHER gifts happened to be loaded alongside it in the same `createBelief` call). It costs more wall-clock in exchange (re-scoring witnessForm once per pair instead of once for all pairs together) — worth it exactly when the alternative is failing to run at all. If a caller genuinely needs several gifts' shares to interact (e.g. `distribution()`'s actual blended output at generation time, where gifts really do compete for the same probability mass), they must be loaded together and this note does not apply.

## `discoverReferents` does not coreference a bare name with its titled form — a caller needing that has to merge, gated, not just union on containment

**`perceiver/text/surfaces.js::discoverReferents`'s name-variant clustering, measured at War and Peace's scale (2,600+ named surfaces), leaves "Andrew" and "Prince Andrew" — obviously the same person — as two entirely separate, unrelated `referent_id`s, along with dozens of other title/bare-name and title/full-name splits for the same characters. This is a real gap in the upstream engine, not something a caller reading its `events` can work around by picking a different option. `scripts/hyperlexicon-definitions-data.mjs::mergeReferents` (commit `b01e057`, same day) is the disclosed, tested local mitigation: union-find over whole-word display containment (`"Andrew"` is a whole-word substring of `"Prince Andrew"`), but ONLY when neither `perceiver/text/pronouns.js::referentGenderEvidence`/`genderClass` contradicts (two referents evidenced as different genders can never be the same person) nor the two names are ever mentioned as two distinct, non-overlapping actors in the same sentence (direct proof the text treats them as different people). Both gates run at the GROUP level so two already-merged clusters can't still fuse transitively through a third. A raw containment merge with neither gate was tried first and measured to catastrophically over-merge: the bare title "Prince" alone fused Pierre, Natásha, Andrew, and Napoleon into one referent by acting as a hub every one of their fuller names contains.**

**Reuse this, don't re-derive it worse — but reusing it is not the whole fix.** A second session, independently building referent-abstracted generation against `belief.js` the same day, hit the identical "Andrew"/"Prince Andrew" split from the generation side (an `abstraction.of(form)` map built only from single-token, unambiguous surfaces had nothing to pool for a character whose titled form is used almost as often as his bare name — 983 "Prince Andrew" vs. 1,143 "Andrew" — while it worked fine for a character like Pierre who is essentially always called by one bare name). Porting `mergeReferents`'s exact algorithm (`sessionReferents`, `referentGenderEvidence`, `genderClass` are all real, importable engine/host exports already; `mergeReferents` itself is presently local to that one script and not yet exported for reuse — a real gap worth closing the next time either script is touched) was the right move over inventing a second heuristic — but ran and STILL did not merge "Andrew" with "Prince Andrew". Traced further: bare "Andrew"'s degree in the containment graph is 33 (`HUB_DEGREE_CAP` is 3), so the same guard that correctly protects "Prince" from over-merging also, correctly by its own logic, refuses to touch "Andrew" — it cannot tell "one real person with many name-forms" from "one hub genuinely touching many different people" from degree alone.

**FIXED, same day, after review.** Filtering that degree count down to referents with real recurrence (`mentions >= 2`) only drops it from 33 to 16, still far over the cap, and the survivors are not noise: `"Rostóvs Prince Andrew"` (165 mentions), `"Bilíbin Prince Andrew"` (52), `"Pfuel Prince Andrew"` (39), `"Drissa Prince Andrew"` (25) — each recurs legitimately, but not because it is a name-variant of Prince Andrew. `extractSurfaces`'s capitalized-run scanner split a sentence's tokens on whitespace and stripped leading/trailing non-letter characters per token, but the run-continuation check (`CAP_TOKEN.test(toks[j])` for consecutive tokens) never looked at what separated two adjacent capitalized tokens in the original text. A comma did not break the run: `"Bilíbin, Prince Andrew's host, ..."` tokenizes to `["Bilíbin", "Prince", "Andrew", ...]`, all three pass `CAP_TOKEN`, and the scanner read them as one uninterrupted 3-word capitalized run — manufacturing "Bilíbin Prince Andrew" as if it were a single name, when it is two different people's names separated only by a comma. This recurred wherever the text lists or juxtaposes capitalized names with only punctuation between them, common enough (52, 39, 25 real occurrences for the three examples above) to keep tripping `HUB_DEGREE_CAP` on any character frequently mentioned alongside others.

**The fix, and what it exposed.** `extractSurfaces` now tracks, per token, whether a run-breaking mark (`,`/`;`/`:`) sat between it and the token before it (read off the SAME raw whitespace-split pass that already strips each token's own edge punctuation, so the two facts can't drift apart), and a capitalized run may always START at any capitalized token but may only EXTEND across one with no break immediately before it. Verified directly against Frankenstein's fixture with `git stash`: `mentions`/`sentences` counts for many candidates shift once the comma-crossing false positives stop being counted, which is expected and correct. But that shift flipped `conformance/host-graph.test.js`'s Henry Clerval assertion — `discoverReferents`'s own clustering loop was greedy first-match, not transitive: "Henry" ⊂ "Henry Clerval" and "Clerval" ⊂ "Henry Clerval" both hold, but "Henry" and "Clerval" never directly corefer with each other, so whether all three (plus "M Clerval") land in one referent had always depended on which of them got admitted first, which in turn depends on relative mention counts — exactly what this fix changed elsewhere in the pool. This was a real, pre-existing fragility the run-scanner fix exposed rather than introduced. Closed by converting `discoverReferents`'s admission loop from greedy first-match to union-find over the same `corefersIndividuated` judgment (the relation itself is unchanged; only how pairwise judgments compose into groups changed) — the identical technique `mergeReferents` already proved out for the analogous title-hub problem, applied here to the relation this file itself computes. Full suite (1,098 tests) passes; Henry Clerval's referent now correctly carries all four surfaces (Clerval, Henry, Henry Clerval, M Clerval) where before the fix it only ever carried two.
