...
**DISCLOSED, NOT FIXED — the deeper cause is one level upstream of `mergeReferents`.** Filtering that degree count down to referents with real recurrence (`mentions >= 2`) only drops it from 33 to 16, still far over the cap, and the survivors are not noise: `"Rostóvs Prince Andrew"` (165 mentions), `"Bilíbin Prince Andrew"` (52), `"Pfuel Prince Andrew"` (39), `"Drissa Prince Andrew"` (25) — each recurs legitimately, but not because it is a name-variant of Prince Andrew. `extractSurfaces`'s capitalized-run scanner (`packages/engine/perceiver/text/surfaces.js`) splits a sentence's tokens on whitespace and strips leading/trailing non-letter characters per token, but the run-continuation check (`CAP_TOKEN.test(toks[j])` for consecutive tokens) never looks at what separated two adjacent capitalized tokens in the original text. A comma does not break the run: `"Bilíbin, Prince Andrew's host, ..."` tokenizes to `["Bilíbin", "Prince", "Andrew", ...]`, all three pass `CAP_TOKEN`, and the scanner reads them as one uninterrupted 3-word capitalized run — manufacturing "Bilíbin Prince Andrew" as if it were a single name, when it is two different people's names separated only by a comma. This recurs wherever the text lists or juxtaposes capitalized names with only punctuation between them, which is common enough (52, 39, 25 real occurrences for the three examples above) to keep tripping `HUB_DEGREE_CAP` on any character who is frequently mentioned alongside others. Not fixed here: `extractSurfaces` is a heavily shared, foundational organ (everything downstream of "which capitalized runs are candidate names" depends on it), and a fix belongs there — the run scanner needs to break at sentence-internal punctuation between tokens, not just strip punctuation off each token's own edges — not as a workaround bolted onto `mergeReferents` or any one caller.
