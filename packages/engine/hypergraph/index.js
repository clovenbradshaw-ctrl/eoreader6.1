...
const refsOf = (value) => {
  ...
  const visit = (v) => {
    if (v == null) return;
    if (typeof v === "string") {
      if (/^(ref|obs|edge|expectation|obligation|frame|pattern|delta|op):/.test(v)) refs.add(v);
      return;
    }
...
export function buildHypergraph(entries = []) {
  ...
  for (const entry of entries) {
    ...
    if (entry.schema === "EOHyperedge@1") {
      for (const p of entry.participants ?? []) addIndex(incident, p.ref, entry.id);
    }
...
export function relevantHypergraphNeighborhood(graph, seeds = [], { maxHops = 3 } = {}) {
  ...
  for (let hop = 0; hop < maxHops && frontier.size; hop += 1) {
    ...
    for (const id of frontier) {
      ...
      if (entry?.schema === "EOHyperedge@1") {
        for (const p of entry.participants ?? []) if (!seen.has(p.ref)) { seen.add(p.ref); next.add(p.ref); }
      }
